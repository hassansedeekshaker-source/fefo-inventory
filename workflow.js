...
function buildSave(t){
 ...
 shell('<div class="card"><h1 id="title"></h1><p>الحفظ يسجل المستند كمسودة فقط. لا يتم تحديث المخزون ولا إنشاء قيد محاسبي حتى الترحيل.</p><div class="head"><label>رقم المستند<input id="docno"></label><label>التاريخ<input id="date" type="date" value="'+today()+'"></label>'+party+'<label>الموقع<select id="site">'+opts(sites,x=>x.name_ar)+'</select></label></div><table><thead><tr><th>الصنف</th><th>الكمية</th><th>سعر/قيمة</th><th>ضريبة/تكلفة</th><th>إجراء</th></tr></thead><tbody id="lines">'+rows+'</tbody></table><button onclick="addLine()">+ إضافة صنف</button><button class="ok" onclick="saveDraft()">💾 حفظ فقط</button><div id="invoiceSummary" style="display:flex;gap:12px;flex-wrap:wrap;margin-top:18px;direction:rtl"><div class="card"><b>قيمة الفاتورة</b><div id="invoiceValue">0.00</div></div><div class="card"><b>إجمالي الضريبة</b><div id="invoiceTax">0.00</div></div><div class="card"><b>إجمالي الفاتورة</b><div id="invoiceTotal">0.00</div></div></div><a class="btn" href="'+(isP?(isR?'purchase-return-post.html':'purchase-post.html'):(isR?'sale-return-post.html':'sale-post.html'))+'">📤 شاشة الترحيل</a><div id="msg"></div></div>');

 if(isP&&!isR){document.querySelector('thead tr').innerHTML='<th>الصنف</th><th>الوحدة</th><th>الكمية</th><th>آخر سعر شراء</th><th>سعر الشراء الجديد</th><th>القيمة المضافة %</th><th>ضريبة القيمة المضافة للوحدة</th><th>قيمة الضريبة</th><th>الإجمالي</th><th>إجراء</th>';setTimeout(()=>{document.querySelectorAll('.item').forEach(purchaseItemChanged);updateInvoiceSummary()},0);}
